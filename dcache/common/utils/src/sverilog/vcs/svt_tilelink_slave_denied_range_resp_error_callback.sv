`ifndef GUARD_TILELINK_SLAVE_DENIED_RESP_ERROR_CALLBACK
`define GUARD_TILELINK_SLAVE_DENIED_RESP_ERROR_CALLBACK

/**
 * Abstract: 
 * class 'tilelink_slave_DENIED_RESP_error_callback' is extended from svt_tilelink_slave_callback class.
 * It implements the post_seq_item_get method to set incorrect d_denied signal response from slave, while accessing user configured denied response range.
 */

class tilelink_slave_denied_resp_error_callback extends svt_tilelink_slave_callback;
  ...
  virtual function void post_seq_item_get(svt_tilelink_slave slave, svt_tilelink_slave_transaction xact, ref bit drop);
    ...
    tilelink_slave_exception.error_kind = svt_tilelink_slave_transaction_exception::RESP_D_DENIED_SIG_ERR;
...
endclass: tilelink_slave_denied_resp_error_callback
`endif //GUARD_TILELINK_SLAVE_DENIED_RESP_ERROR_CALLBACK
