  ...
  virtual task body();
    ...
    `uvm_info(get_type_name(), "dcache sequence starting", UVM_NONE)
		...
		success 	= std::randomize(tag_idx_t,set_idx_t) with {
			...
			((tag_idx_t<<13)+(set_idx_t<<6)+64*(length1+length2)) inside {['h8000_0000:'h7f_ffff_ffff]};
		};
		...
		if(random_hazard) begin
			for(int i=0;i<length2;i++)begin
				...
				fork
					begin
						...
						if(cmd_1 == 0) begin
							dcache_load(addr_t+'h40*i,size_t2,is_signed);
						end
						else if(cmd_1 == 1) begin
							dcache_store(addr_t+'h40*i,wdata,size_t2);
						end
					end

					begin
						if(cmd_2 == 0) begin
							dcache_amo_operation(amo_cmd,addr_t+'h40*i,wdata,size_t1);
						end
						else if(cmd_2 == 1) begin
							dcache_load(addr_t+'h40*i,size_t2,is_signed,1);
						end
						else if(cmd_2 == 2) begin
							dcache_store(addr_t+'h40*i,wdata,size_t2,1);
						end
						else if(cmd_2 == 3) begin
							dcache_partial_mask_store(addr_t+'h40*i,wdata,req_wmask,1);
						end
						else if(cmd_2 == 4) begin
							dcache_amo_operation(amo_cmd,addr_t+'h40*i,wdata,size_t1,1);
						end
					end
				join
			end	
			...
			for(int i=0;i<20;i++)begin
				...
				if(init_state == "T") begin
					dcache_store(addr_t,0,size_t2);
				end
				else if(init_state == "B") begin
					dcache_load(addr_t,size_t2,is_signed);
				end
			end
			...
			for(int i=0;i<length1;i++)begin
				...
				if(cmd1 == 0) begin 	//LOAD
					...
					dcache_store(addr_t,wdata,size_t2);
				end
					...
					if(bypass_cmd == 0) begin
						...
						dcache_store(addr_t,wdata,size_t2,1);
					end
